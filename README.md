...
> **Note**
>
> You can replace `animation_frames.h` with your own animation frames generated using Image2CPP or any SSD1306-compatible bitmap exporter.
...
The sample animation included in this repository was generated using **Image2CPP** and exported as monochrome SSD1306-compatible bitmaps.

The engine renders frames using:

```cpp
display.drawBitmap()
```

This approach maximizes compatibility with animations generated by Image2CPP and other bitmap export tools.

Different exporters may use different memory layouts, byte ordering, or scan directions. Because of this, low-level framebuffer optimizations may produce corrupted, mirrored, shifted, or distorted images if the bitmap format does not exactly match the expected layout.

For this reason, the default implementation prioritizes compatibility and portability over maximum rendering speed.

### Recommended Image2CPP Settings
...
- Store frames in **PROGMEM**

Using different export settings may require modifying the rendering code.

### Important

If an animation appears distorted, mirrored, compressed, or displayed as horizontal/vertical lines, verify that it was exported using SSD1306-compatible settings in Image2CPP.
## License
