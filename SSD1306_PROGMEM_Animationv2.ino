...
static constexpr uint32_t I2C_FREQUENCY_HZ   = 400000UL; // Fast Mode I2C
...
static constexpr uint16_t FRAME_DELAY_MS    = 64;   // ~15.6 FPS
...
private:
    ...
    void renderFrame(uint16_t frameIndex) {
        // Read frame pointer from PROGMEM pointer table
        // pgm_read_ptr() is mandatory on ESP8266 for PROGMEM arrays of pointers.
        // On ESP32 it's a no-op but harmless — keeps the code portable.
        ...
        if (frameData == nullptr) {
            // Corrupted frame table entry — skip this frame silently
            return;
        }

        // Get pointer to the SSD1306 internal display buffer (1024 bytes)
        uint8_t* const displayBuffer = _display.getBuffer();

        if (displayBuffer == nullptr) {
            // Should never happen after successful begin(), but guard anyway
            return;
        }

        // Direct PROGMEM-to-RAM copy: replaces clearDisplay() + drawBitmap()
        // memcpy_P() reads from Flash on both ESP8266 and ESP32.
        memcpy_P(displayBuffer, frameData, FRAME_BYTES);

        // Push buffer to physical display over I2C
        _display.display();
    }
